...
def mock_models(mocker):
    ...
    mock_project.get_project_by_id.return_value = FakeProject()
    mocker.patch("routes.sessions.ProjectModel.create_instance", return_value=mock_project)
    
    mock_session = AsyncMock()
    # The session belongs to proj-1 but we try to access it via proj-2
    mock_session.get_session_scoped.return_value = None
...
def test_get_session_cross_project_fails(client, mock_models):
    response = client.get("/api/v1/nlp/sessions/proj-2/sess-abc", headers={"X-API-Key": "mock-api-key"})
    assert response.status_code == 404
    assert response.json()["detail"]["signal"] == "SESSION_NOT_FOUND"

def test_delete_session_cross_project_fails(client, mock_models):
    response = client.delete("/api/v1/nlp/sessions/proj-2/sess-abc", headers={"X-API-Key": "mock-api-key"})
    assert response.status_code == 404
    assert response.json()["detail"]["signal"] == "SESSION_NOT_FOUND"
